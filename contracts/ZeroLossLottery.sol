...
contract ZeroLossLottery is Ownable {
    // State variables
    IERC20 public sei;
    uint256 public ticketPrice = 1 * 10**18; // 1 SEI (with 18 decimals)
    ...
    // Constructor now accepts the address of SEI token on SEI Testnet
    constructor(address _seiAddress) {
        sei = IERC20(_seiAddress);
    }
    ...
    function buyTickets(uint256 numTickets) external {
        ...
        require(sei.balanceOf(msg.sender) >= totalCost, "Insufficient SEI balance");
        
        // Transfer SEI from user to contract
        bool success = sei.transferFrom(msg.sender, address(this), totalCost);
    ...
    function addInterest(uint256 percentage) external onlyOwner {
        ...
        require(sei.balanceOf(msg.sender) >= interestAmount, "Insufficient admin SEI balance");
        
        // Transfer interest from admin to contract
        bool success = sei.transferFrom(msg.sender, address(this), interestAmount);
    ...
    function claimFunds() external {
        ...
        bool refundSuccess = sei.transfer(msg.sender, refundAmount);
        require(refundSuccess, "Refund transfer failed");
        
        if (prizeAmount > 0) {
            bool prizeSuccess = sei.transfer(msg.sender, prizeAmount);
    ...
    function claimPlatformFee() external onlyOwner {
        ...
        bool success = sei.transfer(owner(), platformFee);
